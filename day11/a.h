...
auto process(monkeys_t monkeys, bool part1, int rounds)
{
    ...
    std::cout << "git counts" << std::endl;
    for(auto& monkey : monkeys){
        counts.push_back(monkey.inspection_count);
    }
    for (int i = 0; i < monkeys.size(); i++) {
        std::cout << "Monkey " << i << " threw " << monkeys[i].inspection_count << " bananas" << std::endl;
    }
    std::cout << " -----------------------------" << std::endl;
...
void a()
{
    auto actual_values = load_input("/home/antoni/dev/aoc22/day11/data.txt");
